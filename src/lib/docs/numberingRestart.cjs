 ...
 *   collapse to a SINGLE numId so Word treats them as one continuous
 *   list (e.g. items 1..6 of a six-paragraph list inside one workplace
 *   share one <w:num> and get numbered 1, 2, 3, 4, 5, 6 — not six
 *   separate "1." starts).
 *
 ...
 *   for any given N is always the FIRST sentinel of N in each iteration.
 *
 ...
 *   3. On `__NUMID_<N>_SLOT_<K!=0>__`: reuse N's current iteration's
 *      mapped numId.
 *
 * The implementation is conservative: it only mutates files if at least
 * one sentinel is present. Templates without sentinels render unchanged.
 *
 * Implemented as CommonJS so that both src/lib/docs/engine.ts (browser
 * generators) and scripts/test-*.js (Node-side regression tests) share
 * exactly one implementation — no risk of drift.
 *
 * @param {import('pizzip')} zip - PizZip instance AFTER docxtemplater render.
 */
function restartListNumberingPerLoop(zip) {
  const docFile = zip.file("word/document.xml");
  const numFile = zip.file("word/numbering.xml");
  if (!docFile || !numFile) return;

  let docXml = docFile.asText();
  if (docXml.indexOf("__NUMID_") === -1) return; // nothing to do

  let numXml = numFile.asText();

  // Find max existing numId in numbering.xml.
  const existingIds = [];
  ...
  // Cache of ilvl arrays per abstractNumId so we don't re-scan
  // numbering.xml for each clone.
  const ilvlsByAbstract = new Map();
  function getIlvlsForAbstract(abstractId) {
    if (ilvlsByAbstract.has(abstractId)) return ilvlsByAbstract.get(abstractId);
    const re = new RegExp(
    ...
    // Fallback: at minimum ilvl=0 must exist for a usable list.
    if (out.length === 0) out.push("0");
    ilvlsByAbstract.set(abstractId, out);
    return out;
  }
  function getOriginalDef(originalNumId) {
  ...
  // Per-origId counter of how many SLOT_0 sentinels (iteration starts)
  // we've seen so far. Iteration #0 is the original iteration; later
  // iterations allocate cloned numIds.
  const seenSlot0Count = new Map();
  // Active mapping per origId for the CURRENT iteration. Set each time
  // we encounter a SLOT_0 sentinel for that origId; non-zero slots read
  // this map to reuse the iteration's mapped numId.
  ...
  docXml = docXml.replace(sentinelRe, function (_match, origIdStr, slotStr) {
    if (slotStr === "0") {
    ...
    // SLOT_0 for this origId. This collapses all sentinels of the same
    // origId within one iteration onto a SINGLE numId.
 ...
 * @returns {number}
 */
function findNumInsertionIndex(numXml) {
...
module.exports = { restartListNumberingPerLoop, findNumInsertionIndex };
